...
EOF
	...
	for f in /docker-entrypoint-initdb.d/*; do
		...
		*.sql)
			echo "$0: running $f"
			/usr/bin/mysqld --user=mysql --bootstrap --verbose=0 --skip-name-resolve --skip-networking=0 <"$f" >/dev/null 2>&1
			echo
			;;
		*.sql.gz)
			echo "$0: running $f"
			# trunk-ignore(shellcheck/SC2094)
			gunzip -c "$f" | /usr/bin/mysqld --user=mysql --bootstrap --verbose=0 --skip-name-resolve --skip-networking=0 <"$f" >/dev/null 2>&1
			echo
			;;
...
echo ' -------------- MySQL Server is ready now and is running successfully ----------------- '
